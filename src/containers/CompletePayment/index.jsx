...
const SuccessIcon =
  <svg width="16" height="14" role="image" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const ErrorIcon =
  <svg width="16" height="16" role="image" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const InfoIcon =
  <svg width="14" height="14" role="image" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const STATUS_CONTENT_MAP = {
  ...
  processing: {
    text: "Seu pagamente está sendo processado.",
    iconColor: "#6D6E78",
    icon: InfoIcon,
    buttonText: 'Voltar para a loja',
    url: '/'
  ...
  return (
    <div className="container">
        <div className="payment-status">
            ...
            <h2 className="status-text status-title">{STATUS_CONTENT_MAP[status].text}</h2>
            {intentId && <div className="details-table">
                <table>
                <tbody>
                    <tr>
                    <td className="TableLabel">id</td>
                    <td className="intent-id TableContent">{intentId}</td>
                    </tr>
                    <tr>
                    <td className="TableLabel">status</td>
                    <td className="intent-status TableContent">{status}</td>
                    </tr>
                </tbody>
                </table>
            </div>}
            {intentId && <a href={`https://dashboard.stripe.com/payments/${intentId}`} className="view-details" rel="noopener noreferrer" target="_blank">Ver Detalhes
                <svg wclassNameth="15" height="14" role="image" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg" style={{paddingLeft: '5px'}}>
            ...
            <a className="retry-button" href={STATUS_CONTENT_MAP[status].url}>
                {STATUS_CONTENT_MAP[status].buttonText} {/*muda o texto de acordo com o status*/}
            </a>
        </div>
    </div>
  );
