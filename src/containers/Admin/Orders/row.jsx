...
//PARA A CRIAÇÃO DESTA TABELA FOI USADO O MUI.COM

export function Row({row, setOrders, orders}) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false)

  async function newStatusOrder(id, status){ //função para atualizar o status do pedido
    try {
      setLoading(true)
      await api.put(`orders/${id}`, {status})

      const newOrders = orders.map(order => order._id === id ? {...order, status} : order)
      //cria um novo array de orders, onde o id do pedido é igual ao id do pedido selecionado
      setOrders(newOrders)
    } catch (error) {
